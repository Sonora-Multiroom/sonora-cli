...
func TestRoutesRunGet_HelpUsesNewGrammar(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "sonora get routes/<route-id>") {
		t.Errorf("expected usage line to name the new get grammar, got stdout:\n%s", stdout.String())
	}
	if strings.Contains(stdout.String(), "sonora routes get") {
...
func TestRoutesRunGet_TooManyArguments(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := routes.RunGet([]string{"a", "b"}, &stdout, &stderr)
...
func TestRoutesRunGet_UnreachableHubURL(t *testing.T) {
	var stdout, stderr bytes.Buffer
	start := time.Now()
	code := routes.RunGet([]string{"route-abc-123", "--hub-url", "http://127.0.0.1:1"}, &stdout, &stderr)
...
func TestRoutesRunGet_VerboseAppendsRawErrorDetail(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := routes.RunGet([]string{"route-abc-123", "--hub-url", "http://127.0.0.1:1", "--verbose"}, &stdout, &stderr)
...
func TestRoutesRunGet_NonVerboseOmitsRawErrorDetail(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := routes.RunGet([]string{"route-abc-123", "--hub-url", "http://127.0.0.1:1"}, &stdout, &stderr)
