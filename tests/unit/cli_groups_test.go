...
func TestGroupsRunList_UnreachableHubURL(t *testing.T) {
	var stdout, stderr bytes.Buffer
	start := time.Now()
	code := groups.RunList([]string{"--hub-url", "http://127.0.0.1:1"}, &stdout, &stderr)
...
func TestGroupsRunList_HelpUsesNewGrammar(t *testing.T) {
	...
	if !strings.Contains(stdout.String(), "sonora get|list groups") {
		t.Errorf("expected usage line to name the new get/list grammar, got stdout:\n%s", stdout.String())
	}
	if strings.Contains(stdout.String(), "sonora groups list") {
...
func TestGroupsRunList_UnexpectedPositionalArgument(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := groups.RunList([]string{"unexpected"}, &stdout, &stderr)
...
func TestGroupsRunList_VerboseAppendsRawErrorDetail(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := groups.RunList([]string{"--hub-url", "http://127.0.0.1:1", "--verbose"}, &stdout, &stderr)
...
func TestGroupsRunList_NonVerboseOmitsRawErrorDetail(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := groups.RunList([]string{"--hub-url", "http://127.0.0.1:1"}, &stdout, &stderr)
