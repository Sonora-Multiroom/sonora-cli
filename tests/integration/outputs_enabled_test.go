...
func mockOutputEnabledServer(t *testing.T, outputsByID map[string]map[string]any) (*httptest.Server, *string) {
	t.Helper()
	var lastBody string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		id := strings.TrimSuffix(strings.TrimPrefix(r.URL.Path, "/api/v2/outputs/"), "/enabled")
		o, ok := outputsByID[id]
		if !ok {
...
func TestOutputsEnable_Success(t *testing.T) {
...
func TestOutputsEnable_JSONOutput(t *testing.T) {
	srv, _ := mockOutputEnabledServer(t, map[string]map[string]any{
		"office-speaker": {"outputId": "office-speaker", "displayName": "Office Speaker", "volume": 40, "muted": false, "available": true, "enabled": false},
	})

	res := runCLI(t, "enable", "outputs/office-speaker", "--hub-url", srv.URL, "--json")
	...
	if decoded.OutputID != "office-speaker" || !decoded.Enabled {
		t.Errorf("unexpected decoded content: %+v", decoded)
	}
}

func TestOutputsEnable_NotFound(t *testing.T) {
	srv, _ := mockOutputEnabledServer(t, map[string]map[string]any{})

	res := runCLI(t, "enable", "outputs/missing-output", "--hub-url", srv.URL)
...
func TestOutputsDisable_MissingIdentifier(t *testing.T) {
	res := runCLI(t, "disable", "outputs/")
...
func TestInputsEnable_StillWorks(t *testing.T) {
