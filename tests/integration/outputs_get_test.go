...
func TestOutputsGet_JSONOutput(t *testing.T) {
	...
	res := runCLI(t, "outputs", "get", "office-speaker", "--hub-url", srv.URL, "--json")
	...
	yamlRes := runCLI(t, "outputs", "get", "office-speaker", "--hub-url", srv.URL)
...
func TestOutputsGet_NotFound(t *testing.T) {
	...
	res := runCLI(t, "outputs", "get", "missing-speaker", "--hub-url", srv.URL)
...
func TestOutputsGet_SuccessYAML(t *testing.T) {
	...
	runCLI(t, "outputs", "get", "office-speaker", "--hub-url", srv.URL)

	for _, id := range []string{"office-speaker", "garage-speaker"} {
		start := time.Now()
		res := runCLI(t, "outputs", "get", id, "--hub-url", srv.URL)
