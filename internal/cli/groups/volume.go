...
const setVolumeUsage = "usage: sonora set groups/<group-id> volume <0-100> [flags]"

// RunSetVolume implements `sonora set groups/<group-id> volume <0-100>`:
// it defines and parses this command's flags, resolves the hub URL, sets
// the named group's volume via the hub, and renders the confirmation to
...
func RunSetVolume(args []string, stdout, stderr io.Writer) int {
	fs := flag.NewFlagSet("set groups volume", flag.ContinueOnError)
	fs.SetOutput(stderr)
	...
	// <group-id>/volume/<value> triple (per the documented invocation
	...
	if len(positional) != 3 {
		...
		case len(positional) < 3:
			fmt.Fprintln(stderr, "error: missing required arguments: <group-id> volume <0-100>")
		default:
	...
	gv, err := hub.SetGroupVolume(context.Background(), client, baseURL, groupID, volume)
	if err != nil {
	...
	if *jsonOut {
		fmt.Fprint(stdout, render.RenderGroupVolumeJSON(*gv))
	} else {
		fmt.Fprint(stdout, render.RenderGroupVolumeYAML(*gv))
	}
	return 0
}
