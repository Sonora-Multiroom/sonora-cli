...
const deleteUsage = "usage: sonora delete inputs/<input-id> [flags]"

// RunDelete implements `sonora delete inputs/<input-id>`: it defines and
// parses this command's flags, resolves the hub URL, deletes the named
// ephemeral input via the hub, and renders the result to stdout. Any
// failure is reported on stderr, never stdout, so scripts piping stdout
...
func RunDelete(args []string, stdout, stderr io.Writer) int {
	fs := flag.NewFlagSet("delete inputs", flag.ContinueOnError)
	fs.SetOutput(stderr)
	...
	if err := hub.DeleteInput(context.Background(), client, baseURL, inputID); err != nil {
	...
	message := fmt.Sprintf("Removed inputs/%s.", inputID)
	if *jsonOut {
		fmt.Fprint(stdout, render.RenderInputDeletedJSON(inputID, message))
	} else {
		fmt.Fprint(stdout, render.RenderInputDeletedYAML(inputID, message))
	}
	return 0
}
