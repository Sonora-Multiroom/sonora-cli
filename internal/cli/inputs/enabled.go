...
// named input's enabled state via the hub, and renders the updated input to
...
func runSetEnabled(verb string, enabled bool, args []string, stdout, stderr io.Writer) int {
	...
	input, err := hub.SetInputEnabled(context.Background(), client, baseURL, inputID, enabled)
	if err != nil {
